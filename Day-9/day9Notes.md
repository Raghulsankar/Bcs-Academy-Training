# Types of for loops

1. for...loop
2. for...of (less complex also less controls)
3. for...in (most complex also more controls)

> for...of only used for dont need index value means use for..of method.

# Function

> Function means "First class citizion"
> HOF - Higher Order Function

# Rules of functions:

1. Function can be passed another function

- function can be passed argument

2. function returning another function
3. function assigned to a avariable.

> if function treated as value then they are vfirst class variable.

### why it is called First class variable?

> 1. assign value in variale

> 2. pass value to argument

> 3. return value from a function

# Lexical Scope

- outside of the fuction scope is called 'lexical scope'

# closure

- (own + lexical scope) is called 'closure'.

# Array methods

1. Array.reverse()
2. Array.includes()
3. Array.slice()
4. Array.split()
5. Array.push() -> end
6. Array.Unshift() -> first
   check website like mdn

## join(), split()

> join() converts array to string.

> split() coverts string to array.
