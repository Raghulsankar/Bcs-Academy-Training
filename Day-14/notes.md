# div

- div is an block element

# span 
 
- span is an inline element

# code quality

# css  variables

> css variables also called css custom variables.

# :root
- root is used for global declare variable.

1. primary color
2. secondry color
3. accent color

> variable declare in css 
> declare variable using css --variablename

--primary=color: 
--secondary-color: 
--accent-color: 

> using root to store font-families, colors,etc to store a varibale .


# css units

1. rem
2. em

1rem = 16px

> rem is used into the root for gloablly declare variable of font size , padding , margin , width , height , borders.


> csss variable can use multiple variations.

# what is the difference between rem and em?

>em wiil not check the root  
> em checks the parent value.


